    ...
    size_t M) 
{
  ...
  state->amplitude_corr = 4.0*beta/sqrt((float)M);
  //singleton :(
  ...
  for (size_t i = 0; i < state->pulse_shape_len; i++) {
    k = (float)i - pulse_shape_len/2 + 1e-5; //should actually use sinc instead of this 1e-5 silliness
...
size_t upsample_convolve(
  ...
  assert(state->edge_symbols);
  for (int i = 0; i < state->overlap; i++) {
    //       how much of the pulse makes it into this block
    for (int k = pulse_shape_len - i * state->M; k < state->pulse_shape_len; k++) { //todo: make sure we're not dereferencing every cycle
  ...
  for (; i < state->overlap; i++) {
    for (int k = pulse_shape_len - (state->overlap + i) * state->M;
